            ...
            <form action="{{route('account.update' , $account->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name"
                        ...
                        value="{{ old('name' , $account->name) }}" placeholder="أدخل اسم المستخدم">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="email" name="email" id="email"
                        ...
                        value="{{ old('email' , $account->email) }}" placeholder="example@email.com">
                    @error('email')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="phone" id="phone"
                           ...
                           value="{{ old('phone' , $account->phone) }}" placeholder="+201000000000">
                    @error('phone')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="role" id="role" class="form-select @error('role') is-invalid @enderror">
                        ...
                        <option value="beneficiary" {{ old('role' , $account->role) == 'beneficiary' ? 'selected' : '' }}>beneficiary</option>
                        <option value="provider" {{ old('role' , $account->role) == 'provider' ? 'selected' : '' }}>provider</option>
                    </select>
                    @error('role')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
